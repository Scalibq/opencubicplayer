...
static const itplayerclass::instrument *plInstr;
static const sampleinfo *plSamples;
static const itplayerclass::sample *plModSamples;

static char plInstShowFreq;

static void itDisplayIns40(short *buf, int n, int plInstMode)
{
...
static void itDisplayIns33(short *buf, int n, int plInstMode)
{
  char col=plInstMode?0x07:"\x08\x08\x0B\x0A"[plInstUsed[n]];
  writestring(buf, 0, col, (!plInstMode&&plInstUsed[n])?"þ##: ":" ##: ", 5);
  writenum(buf, 1, col, n+1, 16, 2, 0);
...
static void itDisplayIns52(short *buf, int n, int plInstMode)
{
  char col=plInstMode?0x07:"\x08\x08\x0B\x0A"[plInstUsed[n]];
...
static void itDisplayIns80(short *buf, int n, int plInstMode)
{
  char col;
  ...
  {
    const itplayerclass::instrument &ins=plInstr[plBigInstNum[n]];
  ...
  {
    const itplayerclass::sample &sm=plModSamples[plBigSampNum[n]];
...
static void itDisplayIns132(short *buf, int n, int plInstMode)
{
  char col;
...
static void itDisplayIns(short *buf, int len, int n, int plInstMode)
{
  switch (len)
  {
  case 33:
...
void itpInstSetup(const itplayerclass::instrument *ins, int nins, const itplayerclass::sample *smp, int nsmp, const sampleinfo *smpi, int, int type, void (*MarkyBoy)(char *, char *))
{
  instnum=nins;
  sampnum=nsmp;
  ...
  for (i=0; i<instnum; i++)
  {
    const itplayerclass::instrument &ins=plInstr[i];
    for (j=0; j<128; j++)
