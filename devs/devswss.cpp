...
//
// Sampler device for Windows Sound System Codecs (AD1848/CS4231 or above)
//
...
extern "C" extern sounddevice smpWinSoundSys;
...
static unsigned char oldmuteleft,
                     oldmuteright,
                     oldlineleft,
                     oldlineright,
                     oldcdleft,
...
static char wssGetCfg()
{
  ...
  if (wssType==2)
  {
    ...
    if (!*ptr++)
      return 0;
    wssIRQ=gusIRQ=strtoul(ptr, 0, 10)&15;
    while ((*ptr!=',')&&*ptr)
      ptr++;
    if (!*ptr++)
      return 0;
...
extern "C" {
  sounddevice smpWinSoundSys={SS_SAMPLER, "Windows Sound System", wssDetect, wssInit, wssClose};
  devaddstruct smpWSSAdd = {wssGetOpt, 0, 0, 0};
  char *dllinfo = "driver _smpWinSoundSys; addprocs _smpWSSAdd";
}