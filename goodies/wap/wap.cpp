...
int LoadPCX(FILE *fp, u8 *outpix, u8 * pal, int* width, int* height);
...
{
 printf("WÅrfelAnimator *PROFESSIONAL* (c) 1998 by Felix Domke\n");
 if(argc!=3)
 {
 ...
 while(curpic<numpics)
 {
  ...
  for(int i=0; i<num; i++)
  {
   ...
   int width, height;
   LoadPCX(fpcx, 0, (u8*)pal, &width, &height);
   if(!curpic)
   {
    memcpy(upal, pal, 768);
   ...
   if(!LoadPCX(fpcx, (u8*)cpic, 0, &width, &height))
   {
